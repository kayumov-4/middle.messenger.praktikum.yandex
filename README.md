...
В рамках Спринта 4 были покрыты тестами следующие ключевые части проекта:

- **Кастомный Router**
  - Методы: `go`, `back`, `forward`
  - Редиректы и обработка `guard`-функций
- **Компоненты страниц**
  - Жизненный цикл `mount` / `unmount`
  - Корректная работа с параметрами и query
- **HTTP-запросы через UseFetch**
  - GET/POST-запросы
  - Формирование URL с query-параметрами
  - Отправка JSON и FormData
...
Проект будет доступен по адресу http://localhost:3000 (или указанному в терминале порту).
